...
def get_device_by_serial(token, serial_number):
    """
    Building out function to retrieve syslog-config. Using requests.get
    """
    url = DNAC_BURL + f"/dna/intent/api/v1/network-device/serial-number/{serial_number}"
...
def main():
    ...
    serial_number = "FOC254001MG"
    device = get_device_by_serial(token, serial_number)
